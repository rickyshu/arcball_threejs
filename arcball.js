...
// Stores information on which object was last clicked store last clicked object;
var currentClicked;
...
function onDocumentMouseDown(event) {
  ...
  //2D coordinates of the mouse, in normalized device coordinates (NDC)---X and Y
  //camera from which the ray should originate
  //Updates the ray with a new origin and direction. => 핵심 기능임
...
var lastInputInside = 1; //extreme한 것을 제외한다.
//mouse move을 통해 실시간으로 mouse의 vector을 추적한다.
//추적된 mouse 좌표를 기반으로 =>
function onDocumentMouseMove(event) {
  ...
  if (
    currentClicked == activeArcball ||
    mode[interactiveBoxes.id] == "ROTATE"
  ) {
    // And the corresponding object exists and is in rotate mode
    if (
      currentDoubleClicked != undefined &&
      mode[currentDoubleClicked.id] == "ROTATE" //이중으로 안전장치를 설계하고
    ) {
      var intersects = raycaster.intersectObject(activeArcball); //만약 intersects를 하면,
      ...
      if (intersects.length > 0) {
        ...
        var objPosition = new THREE.Vector3();
        scene.matrix;
        scene.updateMatrixWorld(); //여기에서 updateMatrixWorld을 왜 하는 거지? 변환된 게 없는데? =? to ensure that the transformation matrices of objects in the scene are up-to-date
        //비록 transform이 없더라도, 위 update 작업은 object을 world coordinates 에서 판단하는 데 있어서 중요한 역할을 한다.
        objPosition.setFromMatrixPosition(intersects[0].object.matrixWorld);
        ...
        //archBall을 hit하지 않았으니까, 거치는 게 없다고 보면 된다.
...
function arcballManipulation(
  inputVector,
  lastInputInsideValue,
  desiredInputInside
) {
...
function executeRotation(axis, angle) {
  if (currentDoubleClicked != interactiveBoxes) {
    ...
    axis.applyQuaternion(temp.conjugate()); //rotate을 사용하지 않고, 그냥 일반적인 방법을 사용하네?

    quaternion.setFromAxisAngle(axis, angle);
    currentDoubleClicked.quaternion.multiplyQuaternions(
      quaternion,
      currentDoubleClicked.quaternion
    );
  } else {
    quaternion.setFromAxisAngle(axis, angle);
    interactiveBoxes.quaternion.multiplyQuaternions(
      quaternion,
      interactiveBoxes.quaternion
    );
  }
}
...
function onDocumentMouseWheel(event) {
  ...
  if (event.deltaY > 0) {
    if (camera.zoom > 0.1) camera.zoom /= 1.25;
  }
  camera.updateProjectionMatrix();
}
...
function animate() {
  ...
  //action of updating the raycaster's position and direction based on the mouse's position on the screen (converted to normalized device coordinates) and the current camera's perspective.
  raycaster.setFromCamera(mouseVector, camera);
  ...
  if (
    ...
    currentClicked != activeArcball &&
    mode[intersect[0].object.id] == "TRANSLATE"
  ) {
